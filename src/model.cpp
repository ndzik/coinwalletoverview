...
namespace cwo {
/********************************** CTORS **********************************/
Model::Model() : Observable(), _running(true), _dbinterval(5), _currency(USD)
{
  ...
  CKBURL = "https://ckb.tokenview.com/en/address/{}";
  loaddatabasefile();
  createdatabase();
}
...
void Model::updatewallets()
{
  std::vector<std::thread> futs;
  {
    ...
    while (it != _wallets.end()) {
      Wallet *w = it->second;
      futs.push_back(std::thread([w](Wallet *wallet) { wallet->update(); }, w));
      ++it;
    }
  }
  for (auto &th : futs)
...
void Model::apikey(const std::string &s) { _apikey = s; }

void Model::stop() { _running = false; }

void Model::updateinterval(int mins) { _dbinterval = mins; }
...
void Model::registerwallet(CRYPTOTYPE t, std::string address)
{
  ...
  { /*** CRITICAL AREA START ***/
    std::unique_lock lock(_mtx);
    _wallets.insert(std::pair<CRYPTOTYPE, Wallet *>(
        t, wf.address(address).cryptotype(t).url(wurl).build()));
  } /*** CRITICAL AREA END ***/
}
...
int Model::writer(char *data, size_t size, size_t nmb, std::string *buffer)
{
  int result = 0;
  if (buffer != NULL) {
...
void Model::selectdatabymin(CRYPTOTYPE t, std::vector<Statistic> *v, int limit)
{
  std::stringstream sql;
  char *err;
  sql << "SELECT * FROM ("
      << " SELECT * FROM " << DBNAME.at(t) << " ORDER BY DATA_TD DESC LIMIT "
      << limit << ")"
...
void Model::selectdatabyhour(CRYPTOTYPE t, std::vector<Statistic> *v, int limit)
{
  std::stringstream sql;
  char *err;
  sql << "SELECT * FROM ("
      ...
      << " AVG(DATA_PRICE)"
      << " FROM " << DBNAME.at(t) << " GROUP BY ACCDT"
...
void Model::selectdatabyday(CRYPTOTYPE t, std::vector<Statistic> *v, int limit)
{
  std::stringstream sql;
  char *err;
  sql << "SELECT * FROM ("
...
std::array<double, 2> Model::minmaxmin(CRYPTOTYPE t, int limit)
{
  ...
  sql << "SELECT MIN(DATA_PRICE) FROM ("
      << "SELECT * FROM " << DBNAME.at(t) << " ORDER BY DATA_TD DESC "
      << "limit " << limit << ");";
  ...
  sql << "SELECT MAX(DATA_PRICE) FROM ("
      << "SELECT * FROM " << DBNAME.at(t) << " ORDER BY DATA_TD DESC "
      << "limit " << limit << ");";
  rc = sqlite3_exec(_db, sql.str().c_str(), &Model::callbackselectsingle, &max,
                    &err);
  if (rc != SQLITE_OK) {
    fprintf(stderr, "SQL error: %s\n", err);
    sqlite3_free(err);
  ...
  for (int i = ETH; i < LASTCRYPTO; ++i) {
    ...
    sql << "CREATE TABLE IF NOT EXISTS " << DBNAME.at(t) << "("
        << "DATA_TD DATETIME PRIMARY KEY NOT NULL,"
           ...
           ");";
    int rc =
        sqlite3_exec(_db, sql.str().c_str(), &Model::callbackempty, 0, &err);
    if (rc != SQLITE_OK) {
      fprintf(stderr, "SQL error: %s\n", err);
      sqlite3_free(err);
    }
