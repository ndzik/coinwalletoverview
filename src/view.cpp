...
/********************************* CTORS ***********************************/
View::View()
    : _state(0), _wait(true), _running(true), _szchanged(true), _updated(true)
{
}
...
void View::update() { _updated = true; }

void View::registermodel(Model *m) { _m = m; }
...
void View::hgraphexpansion() { GRAPHEXPANSION = HORIZONTAL; }

void View::vgraphexpansion() { GRAPHEXPANSION = VERTICAL; }
...
void View::adjustgraphs()
{
  ...
  for (uint32_t ln = 0; ln < bitmask.size() / gpl; ++ln) {
    ...
    for (int32_t j = 0; j < gpl; ++j) {
      if (!calcgraphdimensions(&bitmask, gpl, houtergraph, woutergraph, hratio,
                               wratio, ln, j, begy, begx))
        break;
...
bool View::calcgraphdimensions(std::vector<uint8_t> *bitmask, int gpl,
                               int &houtergraph, int &woutergraph, int hratio,
                               int wratio, uint32_t ln, int32_t j, int &y,
                               int &x)
{
...
void View::displaywalletinfo(WINDOW *win, int h, int w)
{
  ...
  for (uint32_t idx = 1; (idx <= numofwallets); ++idx) {
    ...
    mvwprintw(win, ypos, row * widthofrow + 1, "%s", CRTOS.at(t).c_str());
    wattroff(win, A_BOLD);
    /* Print addr of wallet */
    mvwprintw(win, ypos, row * widthofrow + 1 + 3, " : %s", addr.c_str());
    wattron(win, A_BOLD);
  ...
  for (uint32_t ln = 0; ln < bitmask.size() / gpl; ++ln) {
    ...
    for (int32_t j = 0; j < gpl; ++j) {
      if (!calcgraphdimensions(&bitmask, gpl, houtergraph, woutergraph, hratio,
        ...
        break;
      WINDOW *graphout =
          newwin(houtergraph - 2, woutergraph, 1 + ln * hratio, 1 + j * wratio);
                     ...
                     const std::array<double, 2> range)
{
  ...
  for (int x = maxx; x > LOFFSETIG && i >= 0 && cnt < v.size();
       ++cnt, --x, --i) {
    int y = maxy * (v[i].price - range[0]) / diff;
    y = maxy - y; /* invert axis */
    if (i != 0 && i != v.size() - 1) {
      GRAPH_SYMBOL =
          choosegraphsymbol(v[i - 1].price, v[i].price, v[i + 1].price);
    }
    else {
      GRAPH_SYMBOL = "*";
    }
...
bool View::checkresize() { return ((_ux != _basex) || (_uy != _basey)); }
...
void View::drawmetainfo()
{
  ...
  for (uint32_t i = 0; i < c.size(); ++i) {
    ...
    if (newline) {
      ...
      mvwprintw(coinviewwin, 4 + i * height, x * 0.8 + 1, "1-hour change (%):");
      wattron(coinviewwin, A_BOLD);
      mvwprintw(coinviewwin, 4 + i * height, x * 0.8 + 1 + 18, " %+5.6f",
                change);
      wattroff(coinviewwin, A_BOLD);
    }
    ...
    if (newline) {
      ...
      wattron(coinviewwin, A_BOLD);
      mvwprintw(coinviewwin, 5 + i * height, x * 0.8 + 1 + 18, " %+5.6f",
                change);
      wattroff(coinviewwin, A_BOLD);
    }
    wattroff(coinviewwin, COLOR_PAIR(col));
    ...
    if (newline) {
      ...
      wattron(coinviewwin, A_BOLD);
      mvwprintw(coinviewwin, 6 + i * height, x * 0.8 + 1 + 18, " %+5.6f",
                change);
      wattroff(coinviewwin, A_BOLD);
    }
    wattroff(coinviewwin, COLOR_PAIR(col));
...
const char *View::choosegraphsymbol(double oldp, double curp, double nexp)
{
  ...
  else if ((oldp >= curp && nexp >= curp) || (oldp <= curp && nexp <= curp)) {
    return "_";
  }
  else {
    return "*";
