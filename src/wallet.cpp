...
CRYPTOTYPE Wallet::crypto() { return _cryptotype; }

void Wallet::value(const double &v) { _value = v; }

double Wallet::value() { return _value; }

void Wallet::currency(CURRENCY c) { _currency = c; }

CURRENCY Wallet::currency() { return _currency; }

void Wallet::onehourchange(const double &v) { _onehourchange = v; }
double Wallet::onehourchange() { return _onehourchange; }

void Wallet::onedaychange(const double &v) { _onedaychange = v; }
double Wallet::onedaychange() { return _onedaychange; }

void Wallet::oneweekchange(const double &v) { _oneweekchange = v; }
double Wallet::oneweekchange() { return _oneweekchange; }

/********************** ETHERWALLET ****************************************/
EtherWallet::EtherWallet(std::string_view address, std::string_view walleturl)
...
/********************** NERVOSWALLET **************************************/
NervosWallet::NervosWallet(std::string_view address, std::string_view walleturl)
...
{
  std::regex e("(<div class=\"value\"><div class=\"val\">)"
               "(.*?..*?) CKB(</div>)");
  std::string buffer;
  if (!curlget(buffer))
    return;
